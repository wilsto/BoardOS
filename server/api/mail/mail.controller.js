    ...
    Q()
        ...
        .then(function() {
            ...
            getData.fromTask(req, function(myTasks) {
                ...
                console.log('indexalert', indexalert);
            ...
            _.each(usersList, function(user, indexUser) {
                ...
                _.each(taskList, function(task) {
                    if (_.contains(_.pluck(task.actors, '_id'), user._id.toString())) {
                        ...
                        console.log('task.kpis', task.kpis.length);
                        var allkpis = _.cloneDeep(task.kpis);
                        console.log('allkpis', allkpis.length);
                        // sum and mean of KPI
                        var kpisAlerts = [];
                        ...
                        _.each(kpisAlerts, function(kpisAlert) {
                            ...
                            if (kpisAlert.length > 0) {
                                console.log(' kpisAlert- ', kpisAlert[0].calcul.task);
                        ...
                        _.each(kpisGoals, function(kpisGoal) {
                            console.log('kpisGoal+', kpisGoal.calcul.task);
                        ...
                        console.log('##kpisAlert', sumAlert);
                        console.log('##kpisGoals', meanGoal);
                        ...
                        if (meanGoal > 66 && meanGoal < 133) {
                            textUser += '<span style=\'margin-right:35px;background:green;padding:5px;color:white\'> Performance : ' + meanGoal + '%</span>';
                        } else {
                            textUser += '<span style=\'margin-right:35px;background:#FFA500;padding:5px;color:white\'> Performance : ' + meanGoal + '%</span>';
                        }
                        if (sumAlert === 0) {
                            textUser += '<span style=\'background:#bfbfbf;padding:5px;color:white\'> Alerte : ' + sumAlert + '</span>';
                        } else {
                            textUser += '<span style=\'background:red;padding:5px;color:white\'> Alerte : ' + sumAlert + '</span>';

                        }

                        textUser += '</div>';

                        textUser += '<p style=\'font-size:13px\'> <span style=\'color:grey;\'>Context : </span>' + task.context + ' - <span style=\'color:grey;\'>Activity : </span>' + task.activity + '</p>';
                ...
                var email = new sendgrid.Email({
                    ...
                    from: 'willy' + '.' + 'stophe' + '.' + 'pro' + '@' + 'gmail' + '.' + 'com',
                ...
                sendgrid.send(email, function(err, json) {
                    ...
                    console.log('indexUser', indexUser + '/' + usersList.length);
