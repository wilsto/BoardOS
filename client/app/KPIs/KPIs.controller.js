  ...
  .controller('KPIsCtrl', function ($scope, $http, Auth, actionKPI,categoryKPI,groupByKPI,metricTaskFields) {
    ...
    $scope.save = function() {
      ...
      if (typeof $scope.KPI._id === 'undefined') {
        ...
        $.growl({  icon: 'fa fa-info-circle',  message:'KPI "' + $scope.KPI.name + '" was created'});
      } else {
        $http.put('/api/KPIs/'+ $scope.KPI._id , $scope.KPI);
        $.growl({  icon: 'fa fa-info-circle',  message:'KPI "' + $scope.KPI.name + '" was updated'});
      }
      $scope.load();
      $scope.config = {tab1: true, tab2: false};
    };
      ...
      bootbox.confirm('Are you sure?', function(result) {
        if (result) {
          $http.delete('/api/KPIs/' + KPI._id).success(function () {
              ...
              $.growl({  icon: 'fa fa-info-circle',  message:'KPI "' + KPI.name + '" was deleted'});
          });
        }
      }); 
    }; 
