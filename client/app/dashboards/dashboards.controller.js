...
.controller('DashboardsCtrl', function ($scope, $http, categoryKPI) {
  ...
  $scope.pinDashboard = function(dashboard) {
    ...
    $.growl({  icon: 'fa fa-info-circle',  message:'Dashboard "' + dashboard.name + '" was pinned to your dashboards'});
    $scope.load();
  };

  $scope.$watch('searchText', function() {
    $scope.dashboards = ($scope.searchText.length === 0) ? $scope.alldashboards: _.filter($scope.alldashboards, function(dashboard) {return dashboard.name.toLowerCase().indexOf($scope.searchText.toLowerCase()) >= 0 || dashboard.activity.toLowerCase().indexOf($scope.searchText.toLowerCase()) >= 0 || dashboard.context.toLowerCase().indexOf($scope.searchText.toLowerCase()) >= 0;} );
  });
