(function() {
  'use strict';
  angular.module('angular-radar', []).directive('radarChart', function() {
    return {
      restrict: 'E',
      scope: {
        maxValue: '=',
      ...
      link: function(scope, element, attrs) {
        ...
        scope.render = function(data) {
          ...
          config.maxValue = Math.max(config.maxValue, d3.max(data, function(i) {
          ...
          if (total > 2) {
            ...
            for (var j = 0; j < config.levels; j++) {
              ...
              var drawBasic = gRadar.selectAll('.levels').data(allAxis).enter().append('svg:line').attr('x1', function(d, i) {
              ...
              if (j === config.levels - 1) {
                drawBasic.attr('class', 'line').style('stroke', '#555').attr('transform', 'translate(' + (
                config.size / 2 - levelFactor - config.widthShift) + ', ' + (
                config.size / 2 - levelFactor - config.heightShift) + ')');
              } else {
                drawBasic.attr('class', 'line').style('stroke', '#555').style('stroke-dasharray', ('3, 3')).attr('transform', 'translate(' + (
                config.size / 2 - levelFactor - config.widthShift) + ', ' + (
                config.size / 2 - levelFactor - config.heightShift) + ')');
              }
            }
            ...
            axis.append('text').attr('class', 'legend').text(function(d) {
              return d;
            }).style('font-family', 'Verdana').style('font-size', config.fontSize + 'px').style('text-anchor', 'middle').attr('transform', function(d, i) {
              var p = getVerticalPosition(i, config.size / 2);
              return (p < config.fontSize) ? 'translate(0, ' + (config.fontSize - p) + ')' : '';
            }).attr('x', function(d, i) {
              return getHorizontalPosition(i, config.size / 2, config.legendScale);
            }).attr('transform', 'translate(' + (-config.widthShift) + ', ' + (-config.heightShift) + ')').attr('y', function(d, i) {
              return getVerticalPosition(i, config.size / 2, config.legendScale);
            ...
            data.forEach(function(element, index) {
              ...
              gRadar.selectAll('.area').data([dataValues]).enter().append('polygon').attr('class', 'radar-chart-serie' + series).attr('points', function(d) {
                var str = '';
              ...
              }).style('fill-opacity', config.opacityArea).attr('transform', 'translate(' + (-config.widthShift) + ', ' + (-config.heightShift) + ')').on('mouseover', function(d) {
                var z = 'polygon.' + d3.select(this).attr('class');
                gRadar.selectAll('polygon').transition(200).style('fill-opacity', 0.1);
                gRadar.selectAll(z).transition(200).style('fill-opacity', 0.7);
            ...
            gRadar.append('text').attr('class', 'legend').text(config.tips).style('font-family', 'Verdana').style('font-size', config.fontSize + 'px').style('text-anchor', 'middle').attr('transform', 'translate(' + (
            config.size / 2 - config.widthShift) + ', ' + (
            config.size / 2 - config.heightShift) + ')');
          }
        };
